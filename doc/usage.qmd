...
You can also switch to another theme dynamically by using the "settings" menu in the widget.
...
- `"export"`: button to export the current view to a PNG file
...
::: {.callout-note}
For technical reason, the `center="auto"` argument doesn't take into account data passed to `layer_screengrid` or `layer_heatmap_deck`. It may also not work as intended when used in conjunction with `lazy_zoom`.
:::
...
| [layer_arcs](layers/layer_arcs.qmd)                 | Displays curved or straight arc lines between two taxa.                                                     |
| [layer_arcs_deck](layers/layer_arcs_deck.qmd)       | Displays curved or straight arc lines between two taxa (deck.gl version).                                   |
...
For more information about lazy loading, please take a look at the [large datasets](large_datasets.qmd) documentation page.
